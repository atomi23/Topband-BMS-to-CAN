...
(Beispielbild des Web-Interfaces - V59)
...
Empfohlene Boards:
LILYGO® T-CAN485 (ESP32 Classic)
Plug & Play, automatische Flusskontrolle.
Waveshare ESP32-S3-RS485-CAN (ESP32-S3)
...
CAN H
H
CAN-H (Victron BMS-Can)
CAN L
L
CAN-L (Victron BMS-Can)
...
Laden Sie die aktuelle .bin Datei aus den Releases herunter.
Für LILYGO: VictronGateway_Lilygo_V59.bin
Für Waveshare S3: VictronGateway_WaveshareS3_V59.bin
2. Flashen über Web-Tool
Verbinden Sie das ESP32-Board per USB mit dem PC.
Hinweis Waveshare S3: Halten Sie beim Einstecken die "BOOT"-Taste gedrückt.
Öffnen Sie web.esphome.io oder den Adafruit Web Flasher (Chrome oder Edge Browser).
Klicken Sie auf Connect und wählen Sie den COM-Port.
Wählen Sie die heruntergeladene .bin Datei aus.
Klicken Sie auf Install/Program.
3. Einrichtung
Nach dem Neustart öffnet der ESP32 einen WLAN-Hotspot: Victron-Gateway-Setup.
Verbinden Sie sich mit dem Handy/Laptop (Passwort falls gefragt: 12345678 oder leer lassen).
Ein Fenster sollte sich öffnen ("Anmelden"). Falls nicht, rufen Sie 192.168.4.1 auf.
Klicken Sie auf "Configure WiFi" und geben Sie Ihre WLAN-Daten ein.
Das Gateway startet neu und ist nun unter http://victron-gateway.local erreichbar.
